...
class WatchGo:
    def __init__(self):
        ...
        self.logger = Logger(self.config.base_path)
        self.history = []
    ...
    def process_camera(self):
        ...
        while cv2.waitKey(1) == -1:
            ...
            self._process_video_frame(frame, buf, i, roi, movement_threshold,
                                    still_frames, moving, video_size)
                                    
    def _draw_annotations(self, img, corners, board_state):
        """在图像上绘制标注"""
        # 标注角点
        for corner in corners:
            cv2.circle(img, (int(round(corner[0])), int(round(corner[1]))),
                      6, (0, 0, 255), -1)
    ...
    def _process_video_frame(self, frame, buf, i, roi, movement_threshold, still_frames, moving, video_size):
        """处理视频帧"""
        # 更新缓冲区
        buf[i] = frame
    ...
    def _calculate_movement_threshold(self, corners):
        """计算运动检测阈值"""
        corners_array = np.array(corners).reshape((-1, 1, 2)).astype(np.int32)
        return int(cv2.contourArea(corners_array) / (self.config.board_size ** 2))
    ...
    def test_local_image(self):
        """测试本地图像识别"""
        test_image = os.path.join(self.config.base_path, "test.jpg")
    ...
    def load_board_state(self, filename="board_state.txt"):
        ...
        filepath = os.path.join(self.config.base_path, filename)
        try:
            board_state = np.zeros((self.config.board_size, self.config.board_size), dtype="uint8")
    ...
    def calibrate_camera(self, num_frames=30):
        ...
        try:
            ...
            objp = np.zeros((self.config.board_size * self.config.board_size, 3), np.float32)
            objp[:,:2] = np.mgrid[0:self.config.board_size, 0:self.config.board_size].T.reshape(-1,2)
            ...
            for _ in range(num_frames):
                ...
                ret, corners = cv2.findChessboardCorners(gray, (self.config.board_size, self.config.board_size), None)
    ...
    def save_move(self, board_state):
        ...
        try:
            ...
            # 保存到文件
            game_dir = os.path.join(self.config.base_path, 'games', self.current_game_id)
            os.makedirs(game_dir, exist_ok=True)
            
            # 保存棋盘图片
            self.export_board_image(board_state, os.path.join(game_dir, f'move_{timestamp}.png'))
            
            # 保存局面数据
            with open(os.path.join(game_dir, 'game_record.json'), 'w', encoding='utf-8') as f:
                json.dump({
    ...
    def _export_sgf(self, filepath):
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"(;GM[1]FF[4]CA[UTF-8]AP[WatchGo]SZ[{self.config.board_size}]\n")
    ...
    def analyze_territory(self, board_state):
        ...
        try:
            ...
            def flood_fill(x, y, color):
                if (x < 0 or x >= board_state.shape[0] or 
                    y < 0 or y >= board_state.shape[1] or 
                    visited[x, y]):
                    return 0, set()
        ...
        try:
            ...
            for x in range(board_state.shape[0]):
                for y in range(board_state.shape[1]):
                    if board_state[x, y] > 0:
                        visited = np.zeros_like(board_state, dtype=bool)
                        liberties, stones = count_liberties(x, y, board_state[x, y], visited)
    ...
    def analyze_game_progress(self):
        ...
        try:
            ...
            # 计算时间统计
            timestamps = [datetime.strptime(move['timestamp'], '%Y%m%d_%H%M%S') 
                         for move in self.history]
            if len(timestamps) > 1:
                total_seconds = (timestamps[-1] - timestamps[0]).total_seconds()
                progress['total_time'] = total_seconds
                progress['average_time_per_move'] = total_seconds / (len(timestamps) - 1)
    ...
    def setup_ui_controls(self):
        """设置用户界面控制"""
        try:
            cv2.createTrackbar('亮度', 'camera', 50, 100, 
                lambda x: self.set_camera_parameters(brightness=x))
            cv2.createTrackbar('对比度', 'camera', 50, 100, 
                lambda x: self.set_camera_parameters(contrast=x))
            cv2.createTrackbar('曝光', 'camera', 50, 100, 
    ...
    def auto_adjust_camera(self):
        ...
        try:
            ...
            # 自动调整参数
            if brightness < 100:
                self.set_camera_parameters(brightness=min(self.camera.cap.get(cv2.CAP_PROP_BRIGHTNESS) + 10, 100))
            elif brightness > 200:
                self.set_camera_parameters(brightness=max(self.camera.cap.get(cv2.CAP_PROP_BRIGHTNESS) - 10, 0))
                
            if contrast < 30:
                self.set_camera_parameters(contrast=min(self.camera.cap.get(cv2.CAP_PROP_CONTRAST) + 10, 100))
    ...
    def analyze_move_quality(self, board_state):
        ...
        try:
            ...
            if territory:
                # 评估领地优势
                territory_diff = territory['black_territory'] - territory['white_territory']
            ...
            if dead_stones:
                ...
                analysis['threats'].extend([f"位置({x},{y})的棋子有危险" for x, y in dead_stones])
            ...
            for x in range(board_state.shape[0]):
                for y in range(board_state.shape[1]):
                    if board_state[x][y] == 0:  # 空位
                        ...
                        for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                            nx, ny = x + dx, y + dy
                            if (0 <= nx < board_state.shape[0] and 
                                0 <= ny < board_state.shape[1]):
                                if board_state[nx][ny] > 0:
                                    score += 1
                                    
                        # 如果周围有死子，增加权重
                        if dead_stones and any((nx, ny) in dead_stones 
                            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]
    ...
    def visualize_analysis(self, board_state, analysis_type='influence'):
        ...
        try:
            if analysis_type == 'influence':
                ...
                plt.imshow(patterns['influence'], cmap='RdBu', 
                ...
                output_path = os.path.join(
                    ...
                    'analysis', 
                    f'influence_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
                )
